export const RULES_VERSION = '2026-04-04.apex-new-products.v1';
export const RULES_LAST_UPDATED = '2026-04-04';
...
export const RULES_CHANGELOG_NOTE =
    'Initial New Products matrix for EOD Eval, EOD PA, Intraday Eval, Intraday PA, sizes 25K, 50K, 100K, 150K.';
...
const LAST_VERIFIED_AT = '2026-04-04';
...
function makeEvalRule(productType, accountSize) {
    ...
    return {
        ...
        officialSourceUrl: isEod
            ? [OFFICIAL_SOURCES.newProducts, OFFICIAL_SOURCES.eodEval, OFFICIAL_SOURCES.paActivation]
            : [OFFICIAL_SOURCES.newProducts, OFFICIAL_SOURCES.intradayEval, OFFICIAL_SOURCES.paActivation],
        sourceNotes: isEod
            ? [
                'EOD Evaluation page provides profit target, max drawdown, DLL, max contracts, 30-day access, no consistency, no scaling.',
                'PA activation deadline page provides 7 calendar day activation window.',
            ]
            : [
    ...
    return {
        ...
        sourceNotes: isEod
            ? [
                ...
                'Daily Loss Limit page provides exact PA tiers for max contracts and DLL.',
                'EOD Payouts page provides min trade days, min daily profit, safety net, min balance to request, max payouts, 50% consistency, and minimum payout amount.',
            ]
            : [
                'Intraday PA page provides max drawdown, tier-based scaling, max contract ceiling, DLL tier based, inactivity rule.',
                'Daily Loss Limit page provides exact PA tiers for max contracts and DLL.',
                'Intraday Payouts page provides min trade days, min daily profit, safety net, min balance to request, max payouts, 50% consistency, and minimum payout amount.',
            ],
        lastVerifiedAt: LAST_VERIFIED_AT,
    };
}
...
export function resolveCurrentPaTier(rule, currentBalance) {
    ...
    return getTierForProfit(tiers, currentBalance, rule.accountSize);
}
